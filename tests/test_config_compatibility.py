...
def _parse_mutated_config(
    ...
    mutate(config)
    return parse_config(config, ActiveLearningConfig)
...
def test_s3gfn_rejects_selfies_encoder() -> None:
    ...
    with pytest.raises(
        ValidationError,
        match="S3GFNSampler generates canonical SMILES.*SelfiesTransformerEncoder",
    ):
...
def test_s3gfn_rejects_selfies_oracle() -> None:
    ...
    with pytest.raises(
        ValidationError,
        match=r"S3GFNSampler generates canonical SMILES.*mol_repr='smiles'",
    ):
        _parse_mutated_config(
            "config/molecules/s3gfn_exact.yaml",
...
def test_s3gfn_rejects_non_molecular_oracle() -> None:
    """S3-GFN must be paired with a molecular oracle configuration."""
    with pytest.raises(
        ValidationError,
        match="S3GFNSampler generates canonical SMILES.*no consistent molecular",
    ):
        _parse_mutated_config(
            "config/branin/single_fidelity.yaml",
            lambda config: setattr(
                config,
                "sampler",
                OmegaConf.create({"type": "S3GFNSampler", "n_samples": 4}),
            ),
        )
...
def test_generic_botorch_surrogate_rejects_s3gfn_strings() -> None:
    ...
    with pytest.raises(
        ValidationError,
        match="BoTorchGPSurrogateConfig expects numeric tensor inputs",
    ):
        _parse_mutated_config(
            "config/molecules/s3gfn_exact.yaml",
            lambda config: setattr(
