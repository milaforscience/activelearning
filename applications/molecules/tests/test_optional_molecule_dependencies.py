...
def test_building_minimol_encoder_raises_helpful_error_without_extras() -> None:
    script = _blocked_imports_prelude() + textwrap.dedent(
        """
        from activelearning_molecules.encoders.config import MiniMolSmilesEncoderConfig

        try:
            MiniMolSmilesEncoderConfig().build()
        except ImportError as error:
            message = str(error)
            assert "activelearning-molecules" in message
            assert "uv sync --package activelearning-molecules" in message
        else:
