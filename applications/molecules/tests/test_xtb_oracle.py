...
from activelearning_molecules.plotting import (
...
class TestHelpers:
    ...
    def test_write_best_rdkit_xyz_mmff_failure_is_clean(self, tmp_path):
        ...
        with (
            patch(
                "activelearning_molecules.oracles.xtb.AllChem.MMFFHasAllMoleculeParams",
                return_value=False,
            ),
            patch(
                "activelearning_molecules.oracles.xtb.AllChem.MMFFGetMoleculeProperties",
                return_value=None,
            ),
        ):
...
class TestRunXTB:
    def test_successful_return_code_returns_completed_process(self, tmp_path):
        ...
        with patch(
            "activelearning_molecules.oracles.xtb.subprocess.run",
...
class TestXTBIPEAOracleQuery:
    ...
    def test_query_does_not_log_visualization_by_default(self, oracle: XTBIPEAOracle):
        ...
        with (
            ...
            patch(
                "activelearning_molecules.oracles.xtb.build_xtb_query_molecule_figure"
            ) as build_figure,
        ):
...
class TestXTBMoleculeVisualization:
    ...
    def test_query_grid_is_capped_and_score_ranked(self):
        ...
        with patch(
            "activelearning_molecules.plotting.Draw.MolsToGridImage",
    ...
    def test_query_grid_labels_invalid_molecules(self):
        ...
        with (
            patch(
                "activelearning_molecules.plotting.sf.decoder",
                return_value="",
            ),
            patch(
                "activelearning_molecules.plotting.Draw.MolsToGridImage",
...
class TestXTBScoreFidelityRouting:
    ...
    def _mock_xtb_helpers(self, oracle, vertical_return=2.0, adiabatic_return=1.5):
        ...
        rdkit_patch = patch(
            "activelearning_molecules.oracles.xtb._write_best_rdkit_xyz",
            return_value=fake_xyz,
        )
        opt_patch = patch(
            "activelearning_molecules.oracles.xtb._run_xtb_optimize",
    ...
    def test_empty_decoded_molecule_returns_nan_without_xtb(self, oracle):
        ...
        with (
            patch(
                "activelearning_molecules.oracles.xtb.sf.decoder",
                return_value="",
            ),
            patch(
                "activelearning_molecules.oracles.xtb._write_best_rdkit_xyz"
    ...
    def test_xtb_subprocess_failure_returns_nan(self, tmp_path):
        ...
        with (
            ...
            patch(
                "activelearning_molecules.oracles.xtb._run_xtb",
