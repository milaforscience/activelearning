...
This selector-side division is where selection applies labeling cost. The
built-in multi-fidelity entropy acquisitions return raw information gain
because BoTorch's default cost utility is disabled. If a custom acquisition
already weights its scores by cost, dividing again here compounds the penalty.
