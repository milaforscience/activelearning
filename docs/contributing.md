...
and Sphinx API reference stay in sync. The Sphinx build regenerates the AutoAPI
stubs under `docs_api/source/`; only `docs_api/source/index.rst` is tracked.
...
- The standalone API reference is generated by Sphinx AutoAPI from the Python source in
  `src/`, using the configuration under `docs_api/`. Generated stubs under
  `docs_api/source/` are rebuildable artifacts rather than hand-maintained docs.
