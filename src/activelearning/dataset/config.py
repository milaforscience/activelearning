...
class CSVInitialDataConfig(BaseModel):
    ...
    def _parse_observation(
        ...
        *,
        ...
        if isinstance(self.x_columns, str):
            x_value: str | list[float] = self._read_string(
                row,
                self.x_columns,
                row_number,
            )
    ...
    def _read_fidelity(self, row: dict[str, str], row_number: int) -> int:
        ...
        try:
            ...
            raise ValueError(
                f"CSV {self.path!s} row {row_number} column "
                f"{self.fidelity_column!r} must be an integer fidelity; "
                f"got {raw_value!r}."
            ) from error
...
def _maybe_negate_target(value: float, *, negate_targets: bool) -> float:
    """Optionally negate seeded target values before storing them."""
